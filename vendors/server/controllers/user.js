...
const gitlab = require('../utils/gitlab.js');
...
class userController extends baseController {
  ...
  /**
   * gitlab登录
   * @interface /user/login_by_gitlab
   * @method
   * @category user
   * @foldnumber 10
   * @param {String} email email名称，不能为空
   ...
   *
   */
  async getGitLabAuth(ctx) {
    try {
      const { email, password } = ctx.request.body;
      let gitlabInfo = await gitlab.gitlabQuery(email, password);

      console.log("GitLab授权登录结果：", gitlabInfo);
      if (!gitlabInfo.type || gitlabInfo.token == "") {
        console.log("授权失败！");
        yapi.commons.log("GitLab授权结果：" + gitlabInfo, 'error');
        return (ctx.body = yapi.commons.resReturn(null, 401, "GitLab授权失败"));
      }
      ...
      return (ctx.body = yapi.commons.resReturn(null, 401, 'GitLab授权登录结果异常！'));
    }
  }
