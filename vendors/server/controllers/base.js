...
class baseController {
   ...
   * 判断是否是gitlab登录
   */
  async checkGitLabLogin() {
    // console.log('config', yapi.WEBCONFIG);
    if (!yapi.WEBCONFIG.gitlabLogin) {
      return false;
    } else {
      return yapi.WEBCONFIG.gitlabLogin.enable || false;
    }
  }

  async getLoginTitle() {
    return yapi.WEBCONFIG.loginTitle;
  }
  ...
  async getLoginStatus(ctx) {
    ...
    body.gitlab = await this.checkGitLabLogin();
    body.loginTitle = await this.getLoginTitle();
    console.log("config.json中获取title:", body);
    ctx.body = body;
  }
